...
export default function PatientProcedures() {
  const { user, procedures, error, baseUrl } = useLoaderData<{ user: User, procedures: Procedure[]; error: string; baseUrl: string }>();
  ...
  return (
    <>
      ...
      <AddProcedureModal patientId={patientId || ""} baseUrl={baseUrl} open={addModalOpen} onClose={() => setModalOpen(false)} practitionerId={user.sub} />
